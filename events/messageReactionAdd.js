...
module.exports = async (client, reaction, user) => {
  ...
  if (!userObj) {
    ...
    if (userObj.jeffreyReactions >= threshold) {
      ...
      message.channel.send(`${authorAt} has officially been jeffrified for getting ${userObj.jeffreyReactions} jeffrey votes. The council will now vote...`).then(async msg => {
        msg.react("🎉");
        msg.react("💔");
        msg.react("💀");
        msg.react("✅");
      ...
      try {
        ...
        authorAt.send(`You have been automatically jeffrified for getting ${userObj.jeffreyReactions} jeffrey votes. Please wait for mods to revoke this or for the council will now vote...`);
      } catch {
      ...
      // save asap
      userObj.markModified("reactees").then(() => {
        userObj.markModified("controversialMessages").then(() => {
          userObj.markModified("jeffreyReactions").then(() => {
            userObj.save();
          });
        });
      });
      ...
      if (jeffreyLogsChannel) {
        ...
        for (const messageFDB of topMessages) {
          try {
            ...
            promises.push(message.guild.channels.cache.get(channelID).messages.fetch(messageID).then(async msg => {
